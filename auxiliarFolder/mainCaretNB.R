...
#                                 paste("length","length_after_cleaning_text",
#                                       "X.userName","hashtag","URLs", "emoticon", "emoji",
#                                       "language","interjection",
#                                       paste(names(RData)[21:15000], collapse = "+")
#                                       ,sep = "+",collapse = "+"),sep = "~"))
names <- names(RData)
namesLis <- as.list(names)
names(namesLis) <- namesLis
namesLis <- list.remove(namesLis, "date")
RData <- subset(RData, select = names(namesLis))
...
#TWTID
{
  ...
  twtid.nb.trained <- caret::train(twtid.rec,
                                    data = twtid.train,
                                    method = "nb",
  ...
  twtid.nb.cf <- caret::confusionMatrix(
                          predict(twtid.nb.trained, newdata = twtid.test, type = "raw"),
...
#YTBID
{
  ...
  ytbid.nb.trained <- caret::train( ytbid.rec,
                                    data = ytbid.train,
                                    method = "nb",
...
#WARC
{
  ...
  warc.nb.trained <- caret::train(  warc.rec,
                                    data = warc.train,
                                    method = "nb",
  ...
  warc.nb.cf <- caret::confusionMatrix(
                        predict(warc.nb.trained, newdata = warc.test, type = "raw"),
